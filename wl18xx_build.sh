...
function compat-wireless()
{
	...
	then
		...
		make KLIB_BUILD=${KLIB_BUILD} KLIB=${ROOTFS} -j33 || exit 1
	fi
	...
	then
		...
		cd ${WORK_SPACE}/compat-wireless
		make KLIB_BUILD=${KLIB_BUILD} KLIB=${ROOTFS}  -j33 install-modules
	fi

	if [ x"$stage" = "xclean" ]
	then
		cd $WORK_SPACE/compat-wireless
		make KLIB_BUILD=${KLIB_BUILD} KLIB=${ROOTFS} clean -j33 || exit 1
...
function crda ()
{
	...
	then
		download "http://wireless.kernel.org/download/crda/crda-1.1.1.tar.bz2" "crda-1.1.1.tar.bz2"
                download "http://wireless.kernel.org/download/wireless-regdb/regulatory.bins/2011.04.28-regulatory.bin" "2011.04.28-regulatory.bin"
